...
while (salario != 0):
    ...
    if (salario == 0):
        ...
        while (salario < 0):
            print(f"\nFuncionário n°{funcionario}")
            salario = float(input("Insira o salário novamente: "))
...
for i in range(len(salarios)):
    if (abonos[i] <= 100):
        print(f"Salário = R${salarios[i]:,.2f} - R$ 100.0")
        abonos[i] = 100
        minimo = minimo + 1
        continue

    print(f"Salário = R${salarios[i]:,.2f} - R${abonos[i]:,.2f}")
